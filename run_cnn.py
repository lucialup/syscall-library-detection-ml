...
from src.models.cnn_detector import CNNLibraryDetector
...
def run_cnn_cv(
    ...
    **cnn_params
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(rskf.split(np.arange(len(app_data)), y)):
        ...
        try:
            detector = CNNLibraryDetector(**cnn_params)
...
def main():
    parser = argparse.ArgumentParser(description="1D CNN Library Detector")
    ...
    parser.add_argument("--output", type=Path, default=Path("results/cnn_output.json"), help="Output file")
